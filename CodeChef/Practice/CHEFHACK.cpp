...
class string_int {
    ...
    string_int operator+(uint32_t  a) {
        uint32_t  temp, size = strlen(str+end), carry = 0;
        for (uint32_t  i = 80; i != end-1; i--) {
    ...
    for (int yy = 0; yy < T; yy++) {
        ...
        for (int i = 0; i < N*N; i++) {
            if (!v[i][1]) {
                ...
                if (val != 0)
                    ans = ans+((uint32_t )(val-1));
                else {
                    start = stop = 0;
                    q[stop++] = i;
                    even_or_odd = v[i][0]%2;
                    if (!even_or_odd)
                        ans = ans+((uint32_t )(v[i][0]/2));
                    else
                        ans = ans+((uint32_t )(v[i][0]+3)/2);
                    while(start<stop) {
                        pos = q[start];
                        start++;
                        v[pos][1] = 1;
